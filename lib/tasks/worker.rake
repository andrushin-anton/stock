namespace :worker do
  ...
  task run_d: :environment do
    Item.all().each do |item|
      ...
      ten_days_data = Price.where("date < '" + DateTime.now.strftime('%Y-%m-%d 00:00:00') + "'").order('date desc').first(85)
  ...
  task run_history_d: :environment do
    Item.all().each do |item|
      ...
      ('2015-01-01'.to_datetime.to_i .. DateTime.yesterday.strftime('%Y-%m-%d').to_datetime.to_i).step(1.day) do |date|
        ten_days_data = Price.where('date <= ?', Time.at(date).to_datetime).order('date desc').first(85)
        if ten_days_data.length > 1
          # Loop through all of the existing pattens
          Pattern.all().each do |pattern|
            if pattern.respond_to? pattern.name
